/**
 * Announcement Edit Page JavaScript
 */
...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    ...
    if (targetAudience) {
      ...
      const currentValue = targetAudience.value;
      if (currentValue === 'departments') {
      ...
      } else if (currentValue === 'specific_users') {
      ...
      targetAudience.addEventListener('change', function () {
        ...
        audienceSelections.forEach(selection => {
          ...
          // Clear selections only if changing to a different type
          if (!selection.style.display.includes('block')) {
    ...
    if (statusSelect && publishDateInput) {
      statusSelect.addEventListener('change', function () {
        if (this.value === 'scheduled') {
          ...
          const label = publishDateInput.closest('.mb-3').querySelector('.form-label');
          if (label && !label.innerHTML.includes('*')) {
            label.innerHTML = 'Publish Date <span class="text-danger">*</span>';
          }
        } else {
          publishDateInput.required = false;
          const label = publishDateInput.closest('.mb-3').querySelector('.form-label');
          if (label) {
            label.innerHTML = 'Publish Date';
          }
        }
      });

      // Trigger on load if status is scheduled
