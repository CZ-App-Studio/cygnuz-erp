      ...
      <div class="row">
            ...
            <div class="card-body">
              <div class="mb-3">
                ...
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" 
                       placeholder="{{ __('Enter announcement title') }}" value="{{ old('title', $announcement->title) }}" required>
                @error('title')
              ...
              <div class="mb-3">
                ...
                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" 
                          rows="3" placeholder="{{ __('Brief description of the announcement') }}" required>{{ old('description', $announcement->description) }}</textarea>
                @error('description')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="mb-3">
                ...
                <div id="editor-container">
                  <div id="quill-editor" style="height: 300px;">{!! old('content', $announcement->content) !!}</div>
                </div>
              ...
              <div class="row">
                <div class="col-md-6 mb-3">
                  ...
                  <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                    <option value="general" {{ old('type', $announcement->type) == 'general' ? 'selected' : '' }}>{{ __('General') }}</option>
                    <option value="important" {{ old('type', $announcement->type) == 'important' ? 'selected' : '' }}>{{ __('Important') }}</option>
                    <option value="event" {{ old('type', $announcement->type) == 'event' ? 'selected' : '' }}>{{ __('Event') }}</option>
                    <option value="policy" {{ old('type', $announcement->type) == 'policy' ? 'selected' : '' }}>{{ __('Policy') }}</option>
                    <option value="update" {{ old('type', $announcement->type) == 'update' ? 'selected' : '' }}>{{ __('Update') }}</option>
                  </select>
                  @error('type')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
              ...
              <div class="mb-3">
                ...
                @if($announcement->attachment)
                  <div class="alert alert-info mb-2">
                    ...
                    <a href="{{ Storage::url($announcement->attachment) }}" target="_blank">{{ basename($announcement->attachment) }}</a>
                  </div>
                @endif
            ...
            <div class="card-body">
              <div class="mb-3">
                ...
                <select class="form-select @error('target_audience') is-invalid @enderror" id="target_audience" name="target_audience" required>
                  <option value="all" {{ old('target_audience', $announcement->target_audience) == 'all' ? 'selected' : '' }}>{{ __('All Employees') }}</option>
                  <option value="departments" {{ old('target_audience', $announcement->target_audience) == 'departments' ? 'selected' : '' }}>{{ __('Specific Departments') }}</option>
                  <option value="teams" {{ old('target_audience', $announcement->target_audience) == 'teams' ? 'selected' : '' }}>{{ __('Specific Teams') }}</option>
                  <option value="specific_users" {{ old('target_audience', $announcement->target_audience) == 'specific_users' ? 'selected' : '' }}>{{ __('Specific Users') }}</option>
                </select>
              ...
              <div class="mb-3 audience-selection" id="departments-selection" style="{{ old('target_audience', $announcement->target_audience) == 'departments' ? '' : 'display: none;' }}">
                ...
                <select class="form-select select2 @error('departments') is-invalid @enderror" id="departments" name="departments[]" multiple>
                  @foreach($departments as $department)
                    <option value="{{ $department->id }}" {{ in_array($department->id, old('departments', $announcement->departments->pluck('id')->toArray())) ? 'selected' : '' }}>
                      {{ $department->name }}
                    </option>
                  @endforeach
                </select>
              ...
              <!-- Teams Selection -->
              <div class="mb-3 audience-selection" id="teams-selection" style="{{ old('target_audience', $announcement->target_audience) == 'teams' ? '' : 'display: none;' }}">
                ...
                <select class="form-select select2 @error('teams') is-invalid @enderror" id="teams" name="teams[]" multiple>
                  @foreach($teams as $team)
                    <option value="{{ $team->id }}" {{ in_array($team->id, old('teams', $announcement->teams->pluck('id')->toArray())) ? 'selected' : '' }}>
                      {{ $team->name }}
                    </option>
                  @endforeach
                </select>
              ...
              <!-- Users Selection -->
              <div class="mb-3 audience-selection" id="users-selection" style="{{ old('target_audience', $announcement->target_audience) == 'specific_users' ? '' : 'display: none;' }}">
                ...
                <select class="form-select select2 @error('users') is-invalid @enderror" id="users" name="users[]" multiple>
                  @foreach($users as $user)
                    <option value="{{ $user->id }}" {{ in_array($user->id, old('users', $announcement->users->pluck('id')->toArray())) ? 'selected' : '' }}>
        ...
        <div class="col-12 col-lg-4">
            ...
            <div class="card-body">
              <div class="mb-3">
                ...
                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                  <option value="draft" {{ old('status', $announcement->status) == 'draft' ? 'selected' : '' }}>{{ __('Draft') }}</option>
                  <option value="published" {{ old('status', $announcement->status) == 'published' ? 'selected' : '' }}>{{ __('Publish Now') }}</option>
                  <option value="scheduled" {{ old('status', $announcement->status) == 'scheduled' ? 'selected' : '' }}>{{ __('Schedule') }}</option>
                  <option value="expired" {{ old('status', $announcement->status) == 'expired' ? 'selected' : '' }}>{{ __('Expired') }}</option>
                  <option value="archived" {{ old('status', $announcement->status) == 'archived' ? 'selected' : '' }}>{{ __('Archived') }}</option>
                </select>
                @error('status')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control flatpickr-datetime @error('publish_date') is-invalid @enderror" 
                       ...
                       value="{{ old('publish_date', $announcement->publish_date ? $announcement->publish_date->format('Y-m-d H:i') : '') }}">
                @error('publish_date')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control flatpickr-datetime @error('expiry_date') is-invalid @enderror" 
                       ...
                       value="{{ old('expiry_date', $announcement->expiry_date ? $announcement->expiry_date->format('Y-m-d H:i') : '') }}">
                @error('expiry_date')
                  <div class="invalid-feedback">{{ $message }}</div>
                @enderror
              </div>
              ...
              <div class="form-check form-switch mb-3">
                <input class="form-check-input" type="checkbox" id="requires_acknowledgment" name="requires_acknowledgment" value="1" {{ old('requires_acknowledgment', $announcement->requires_acknowledgment) ? 'checked' : '' }}>
          ...
          <div class="card mb-4">
            ...
            <div class="card-body">
              <div class="form-check form-switch mb-3">
                <input class="form-check-input" type="checkbox" id="send_notification" name="send_notification" value="1" {{ old('send_notification', $announcement->send_notification) ? 'checked' : '' }}>
                <label class="form-check-label" for="send_notification">{{ __('Send in-app notification') }}</label>
              </div>

              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="send_email" name="send_email" value="1" {{ old('send_email', $announcement->send_email) ? 'checked' : '' }}>
          ...
          <div class="card mb-4">
            ...
            <div class="card-body">
              <div class="d-flex justify-content-between mb-2">
                <span>{{ __('Total Reads:') }}</span>
                <strong>{{ $announcement->reads->count() }}</strong>
              </div>
              <div class="d-flex justify-content-between mb-2">
                <span>{{ __('Read Percentage:') }}</span>
                <strong>{{ $announcement->read_percentage }}%</strong>
              </div>
              @if($announcement->requires_acknowledgment)
                <div class="d-flex justify-content-between">
                  <span>{{ __('Acknowledged:') }}</span>
                  <strong>{{ $announcement->reads->where('acknowledged', true)->count() }}</strong>
                </div>
              @endif
            </div>
          </div>
          ...
          <div class="card">
            <div class="card-body">
              <button type="submit" class="btn btn-primary w-100 mb-2">
                <i class="bx bx-save me-1"></i> {{ __('Update Announcement') }}
              </button>
