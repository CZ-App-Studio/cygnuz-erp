...
namespace Modules\AICore\app\Providers;
...
class AICoreServiceProvider extends ServiceProvider
{
    ...
    protected string $name = 'AICore';

    protected string $nameLower = 'aicore';
    ...
    {
        if (class_exists(\App\Services\Settings\SettingsRegistry::class)) {
            $settingsRegistry = $this->app->make(\App\Services\Settings\SettingsRegistry::class);
            $settingsRegistry->registerModule('AICore', [
                'handler' => \Modules\AICore\app\Settings\AICoreSettings::class,
                'permissions' => ['aicore.settings.manage'],
                'name' => __('AI Core Settings'),
                'icon' => 'bx bx-brain',
                'description' => __('Configure AI providers, models, and usage settings'),
            ]);
        }
    }
