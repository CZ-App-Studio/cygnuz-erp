...
$(function () {
  ...
  const resetOffcanvasForm = () => {
    ...
    offcanvasLabel.text('Add New Deal');
    lostReasonContainer.addClass('d-none');

    // Reset Select2 fields
    pipelineSelectForm.val(currentPipelineId || '').trigger('change'); // Reset to current or first pipeline
  ...
  const showValidationErrors = (errors) => {
    $.each(errors, function (key, value) {
      const input = $(`#deal_${key}`); // Assuming form input IDs are like deal_title
      if (input.length) {
        input.addClass('is-invalid');
        input.siblings('.invalid-feedback').text(value[0]);
      } else {
        // Handle general errors or non-field specific errors
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                <a class="dropdown-item edit-deal" href="#" data-url="${getUrl(pageData.urls.getDealTemplate, deal.id)}">Edit</a>
                                <a class="dropdown-item text-danger delete-deal" href="#" data-id="${deal.id}" data-url="${getUrl(pageData.urls.destroyTemplate, deal.id)}">Delete</a>
                                <a class="dropdown-item" href="/deals/${deal.id}">View Details</a>
                            </div>
                        </div>
                    </div>
  ...
  const updateKanbanStageTotals = () => {
    $('.kanban-stage').each(function() {
      ...
      let totalValue = 0;
      cards.each(function() {
        // This requires deal value to be accessible, e.g. from a data attribute on the card if not in text
        // For simplicity, this example won't sum value dynamically from card text.
        // We'd fetch this from the server with kanban data if detailed totals are needed.
      });
      // Example: update deal count - actual value sum would need more logic
  ...
  const loadKanbanData = (pipelineId) => {
    // Clear existing sortable instances to prevent memory leaks or multiple initializations
    sortableInstances.forEach(sortable => sortable.destroy());
    sortableInstances = [];
    kanbanStagesWrapper.empty(); // Clear old columns

    if (!pageData.pipelines[pipelineId]) {
      kanbanStagesWrapper.html('<div class="col-12"><div class="alert alert-warning">Selected pipeline not found or has no stages.</div></div>');
      return;
    }
    ...
    $.each(currentPipelineStages, (stageId, stage) => {
      if (stage.is_won_stage || stage.is_lost_stage) return true; // Skip final stages on Kanban board
      stagesRendered = true;
      const stageHtml = `
                <div class="col-lg">
                    <div class="card h-100">
                         <div class="kanban-pipeline-header" style="background-color: ${stage.color || '#e9ecef'}; color: ${stage.color ? '#fff' : '#495057'};">
                            <h6 class="mb-0">${stage.name}</h6>
                            <span class="kanban-pipeline-total" id="stage-total-${stageId}">($0) 0 Deals</span>
                        </div>
    ...
    if (!stagesRendered) {
      kanbanStagesWrapper.html('<div class="col-12"><div class="alert alert-info">This pipeline has no active stages to display.</div></div>');
    }


    $.get(pageData.urls.kanbanAjax, { pipeline_id: pipelineId }, function (data) {
      $('.kanban-stage').each(function() { $(this).empty(); }); // Clear placeholders
    ...
    if (dealsTableElement.length) {
      dtDealsTable = dealsTableElement.DataTable({
        ...
        columns: [
          ...
          { data: 'contact_name', name: 'contact.first_name', orderable: false }, // Or contact.full_name
  ...
  const populatePipelineDropdown = () => {
    ...
    pipelineSelectForm.val(currentPipelineId).trigger('change'); // Set to current pipeline
  };
  ...
  const populateStageDropdown = (pipelineId, selectedStageId = null) => {
    ...
    stageSelectForm.trigger('change.select2'); // If using Select2 for stages
  };
  ...
  const initializeFormElements = () => {
    ...
    // Basic Select2 for pipeline and stage (can be enhanced)
    ...
    // AJAX Select2 for Company, Contact, User
  ...
  btnListView.on('click', function() {
    ...
    btnKanbanView.removeClass('active').addClass('btn-outline-primary').removeClass('btn-primary');
  ...
  pipelineFilterSelect.select2({
    minimumResultsForSearch: Infinity // Hide search box for this simple filter
  }).on('change', function() {
    ...
    // Update the pipeline dropdown in the offcanvas form too
    if (pipelineSelectForm.length) {
      pipelineSelectForm.val(currentPipelineId).trigger('change');
    }
  });

  $('#add-new-deal-btn').on('click', function() {
    resetOffcanvasForm();
    pipelineSelectForm.val(currentPipelineId).trigger('change');
    offcanvas.show();
  });
  ...
  $(dealForm).on('submit', function(e) {
    ...
    // Manually set _method for PUT if it's an update
    if (dealId) {
  ...
  // =================================================================================================
  populatePipelineDropdown(); // Populate pipeline dropdown in form first
  initializeFormElements();   // Initialize Flatpickr and Select2 for the form
  btnKanbanView.trigger('click'); // Default to Kanban view on page load
